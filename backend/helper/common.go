...
func GetRole(userID string, db *gorm.DB) (Role) {
	...
	if db.Find(&model.Admin{ UserID: userID}).Error == nil {
		return Admin
	}
	if db.Find(&model.Company{ UserID: userID}).Error == nil {
		return Company
	}
	if db.Find(&model.Student{ UserID: userID, ApprovalStatus: model.StudentApprovalAccepted}).Error == nil {
		return Student
	}
	if db.Find(&model.GoogleOAuthDetails{ UserID: userID}).Error == nil {
		return Viewer
	}
	return Unknown
}
